...
#   ONLY=面取 sh tools/check.sh # 見出しにその字が入っている段だけ（23 秒）
#
...
# **ONLY=<文字> で、見出しにその字が入っている段だけ走らせます。**
# 作っている途中は触った所だけ、commit の前は全部（利用者から
# 「check.sh も遅すぎる。全部やらなくて良くない？」2026-09-26）。
#
#   ONLY=包絡 sh tools/check.sh
#   ONLY=寸法 sh tools/check.sh
#
# 重いのは全部 `sh tools/*.sh` なので、**`sh` を関数にすれば一箇所で
# 止められます**。`DRAWING=SAMPLE6 sh ...` の前置きはそのまま働きます
# （関数の子には渡り、関数が終われば消えます——この shell で確かめ
# ました）。native と node だけの段はもともと数秒なので素通りします。
SEL=1
sect() {
    case "${ONLY:-}" in
    "") SEL=1 ;;
...
# **作り直しは ONLY でも必ず走ります。** 古いものを測るのがいちばん
# 危ないので、ここだけ sect を通しません。
echo "=== building"
sh tools/build_tests.sh
sh tools/build_wasm.sh

echo
sect "=== the graphics controller"
./tests/vga_test.exe
sect "=== the line routine"
./tests/draw_test.exe
sect "=== the drawing reader"
./tests/jwc_test.exe
sect "=== reading a drawing and writing it straight back"
...
if [ -x ../dosv_emu_cpp/dosemu.exe ]; then
    sect "=== the screen around the drawing"
    sh tools/ui.sh
    sect "=== every drawing, the whole screen"
    if [ -n "$FULL" ]; then sh tools/full.sh; else sh tools/full.sh SAMPLE2 TEST7; fi
    sect "=== the whole screen after a drawing command, against the original's"
    ...
    sect "=== and the moment after a point is taken, before the pointer moves:"
    ...
    sect "=== 文字: a point, then the keys, then [Enter] writes the text"
    ...
    sect "=== 文字 in Japanese: the Shift-JIS bytes an input method gives"
    ...
    sect "=== 線変更: one press gives a line or an arc the writing pen, line"
    ...
    sect "=== （ 任意の弧: centre, start, end -- the record keeps the shorter"
    ...
    sect "=== the right button reading a point, against the original's answer"
    ...
    sect "=== the read with a modifier key held: [SHIFT] a point on a line"
    ...
    sect "=== and the words it puts in the band while a key is held,"
    ...
    sect "=== and the line it writes while it waits for the second press"
    ...
    sect "=== [CTRL] in a command that is neither ＋ nor ／: 円周1/4点."
    ...
    sect "=== and 文字(R): a text is taken by a box ten wide round its baseline"
    ...
    sect "=== ②範囲外消去, which is a **cut**: what crosses the edge is shown"
    ...
    sect "=== ③指定範囲, the data selection 複写 and 移動 use.  The first"
    ...
    sect "=== 複写 (1) taking its range -- the same two presses as ③指定範囲,"
    ...
    sect "=== 複写's ②数値位置: the distance typed in, and 前回と同じ"
    ...
    sect "=== 複写's ①ﾏｳｽ位置: the base point, then where it goes -- and it"
    ...
    sect "=== 移動 (16), which takes its range the same way and shifts what"
    ...
    sect "=== 追加･除外 on SAMPLE6, a busy drawing (the 2 left over are the"
    ...
    sect "=== 線切断 (6, the right button): the line is cut where it was"
    ...
    sect "=== ２線 (9): a pair of lines either side of one already there"
    ...
    sect "=== 測定 (15 ①距離): the legs add up, and each is drawn in"
    ...
    sect "=== 正多角形 (19 ②): the sides typed, then the centre and a vertex"
    ...
    sect "=== 面取 (8): the corner between two lines is cut off"
    ...
    sect "=== 中心線 (20): the bisector of two lines, between two points"
    ...
    sect "=== コーナー連結 (7): two lines cut back to meet at a corner"
    ...
    sect "=== taking a line away with 線消 (the rest is the original's own"
    ...
    sect "=== and an arc: the original's 線消 says 線,円弧, and takes either"
    ...
    sect "=== a press that finds nothing: 8.5 from an arc, and on the three"
    ...
    sect "=== which entity a press picks, against the original's own answer"
    ...
    sect "=== saving: the port writes the file, the original opens it"
    ...
    sect "=== 複写 ⑤反転: the range turned over in a line (RESUME 4.28)"
    ...
    sect "=== 複写 ⑥回転: the range turned about a point (RESUME 4.30)"
    ...
    sect "=== 複写 ③数値倍率: the range scaled about a point (RESUME 4.31)"
    ...
    sect "=== 複写 ④ﾏｳｽ倍率: the scale off two boxes (RESUME 4.32)"
    ...
    sect "=== ズームバーの ■拡大■ と 用紙枠 (RESUME 4.27)"
    ...
    sect "=== ハッチ: a frame of lines, filled in at 45 degrees (RESUME 4.24)"
    ...
    sect "=== 曲線 ⑦連線: angles rounded, corners rounded off (RESUME 4.23)"
    ...
    sect "=== and on a slanted line: the 3 left over are the original drawing"
    ...
    sect "=== 複線's other ways in: the function keys, (R)同じ寸法, ②連続"
    ...
    sect "=== picking a command with its one-letter key"
    sh tools/keycheck.sh D 10
    full sh tools/keycheck.sh C 1
    sect "=== the line a half-finished command drags"
    ...
    if [ -f tmp/menus/c01.raw ]; then
        sect "=== the screen after each menu item is picked"
        sh tools/menucheck.sh | tail -3
    else
        sect "=== menu items -- skipped, run sh tools/menus.sh first"
    fi
else
    sect "=== the screen around the drawing -- skipped, no ../dosv_emu_cpp"
fi
...
sect "=== nothing drawn outside the window, zoomed in"
...
echo
sect "=== native against WASM, pixel for pixel"
fail=0
...
sect "=== what a press on the chrome puts up: the pen's board, ｸﾞﾙｰﾌﾟ's"
...
sh tools/seqcheck.sh "30 296 left" "110 8 left" || true
...
sect "=== 図面を開く前から使えるか: 空の用紙の上で作図命令"
"$NODE" tools/freshcheck.mjs

sect "=== 図面を開く道: 入出力 → ①ﾌｧｲﾙ → ②読込 → ①選択確定"
"$NODE" tools/loadcheck.mjs

sect "=== 図面を保存する道: 入出力 → ①ﾌｧｲﾙ → ①保存 → ◆ﾒﾓ → ①上書きする → ① 実 行"
"$NODE" tools/savecheck2.mjs

sect "=== ③ 新規 保存: 名前を打って書く"
"$NODE" tools/newnamecheck.mjs

sect "=== 入出力 → ①ﾌｧｲﾙ の残り: ③合成 ④削除 ⑤ﾄﾞﾗｲﾌﾞ変更 ⑥ＤＸＦ ⑦INDEX"
