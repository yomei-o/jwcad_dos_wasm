...

{
    ...
    printf 'mouse 30 280\nwait 2000000\ndown left\nwait 2000000\nup left\nwait %s\n' "$WAIT"
    printf 'mouse 100 8\nwait 2000000\ndown left\nwait 2000000\nup left\nwait %s\n' "$WAIT"
    # WIDTH= answers ②枠幅 with another size, so the same shapes can be
    # measured twice and the X rule told apart from the Y one.
    if [ -n "$WIDTH" ]; then
        printf 'mouse 300 8\nwait 2000000\ndown left\nwait 2000000\nup left\nwait %s\n' "$WAIT"
        printf 'type %s\nwait 4000000\nkey enter\nwait %s\n' "$WIDTH" "$WAIT"
    fi
    printf 'wait %s\nshot ../jwcad_dos_wasm/tmp/tg/list%s.raw\n' "${TAIL:-120000000}" "${WIDTH:-}"
} > tmp/tg/s.txt
...
DOSEMU_BP=+10a9:07dc DOSEMU_BPN=400000 \
    ...
    tmp/tg/root/JW_CADV.EXE SAMPLE0.JWC 2>/dev/null > "tmp/tg/lines${WIDTH:-}.txt" || true
...
awk '{x0=strtonum("0x"$8); y0=strtonum("0x"$9);
      ...
      if (x0 >= 122 && x0 <= 380 && y0 >= 16 && y0 <= 63 && y1 >= 16 && y1 <= 63)
          printf "  (%d,%d)-(%d,%d) col=%d type=%s\n", x0, y0, x1, y1,
                 strtonum("0x"$12), $14}' "tmp/tg/lines${WIDTH:-}.txt" | tail -30
echo "--- how many line calls in each cell row ---"
awk '{y0=strtonum("0x"$9); x0=strtonum("0x"$8);
      if (x0 >= 122 && y0 >= 16 && y0 < 464) print int(y0/48)}' "tmp/tg/lines${WIDTH:-}.txt" \
    | sort -n | uniq -c
