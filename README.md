...
（一文字キーでも選べます）。**作図コマンドが 20 個動きます**——＋ ／ □ ○
「（」複線 線消 点 消去 線変更 文字 複写 移動 線伸縮 線切断 コーナー連結
面取 ２線 中心線 分割 正多角形。引いた図面は**保存**できて、本物のＪＷ＿ＣＡＤ がそれを
...
**いまできていること: 画面と、図面の読み書きと、作図コマンド 20 個と、読取と、日本語入力。**
...
まだ無いのは残りの作図コマンド（変形・寸法・ハッチ・図形ほか 10 個）と、
＋ と ／ での [CTRL]（点ではなく向きの拘束）と、日本語入力です。
**ゆくゆくは [dosbox_wasm](https://github.com/yomei-o/dosbox_wasm) と同じように
PDF が出せるところまで**——ただし作図が一通り動いてからです。
...
13. **作図コマンドを 1 つずつ** —— いま 20 個。残り 10 個
    （寸法・測定・変形・ハッチ・曲線・円線接・図形・文編集・
    ｵﾌﾟｼｮﾝ・入出力）
14. PDF 出力 —— [dosbox_wasm](https://github.com/yomei-o/dosbox_wasm) と
    同じように。作図が一通り動いてから
