...
まだ無いのは 変形（17）の残り（②包絡処理変形はＬ型・Ｔ型・＋型・柱／壁・範囲内消去まで、残るのは外形線と線連結、④線記号変形・⑤外部）、曲線（23）の ⑤手書線（と ⑥連続弧 の ①接する弧･線 指定）、
寸法の ②円周 の ②連続始点指示、
帯の 電卓・範囲記憶・ｵﾌｾｯﾄ・HELP、
...
17. ~~入出力のファイル操作~~ 済 — 保存・読込・③合成・④削除・⑦INDEX・
    ⑥ＤＸＦ・⑤新規図面・④自動保存の盤。ページからの上げ下ろしも
    `.JWC` と `.DXF` の両方
18. ~~図形（27）~~ 済 — ①登録 の書く `.JWK` は本物とバイト単位で一致、
    ②読込 で図面に置ける
19. ~~寸法（14）~~ ほぼ済 — ①横方向・②縦方向・③任意方向・⑤寸法値・⑥点・
    ⑦矢印・⑨設定・小数点以下の桁・累寸・連続入力。⑨設定 の値も図に効く。
    ②半径・③直径・⑧値変・④円･角（①円径・②円周・
    ③角度【２点間】【２線間】）と ⑤一括 も。残るのは
    ②円周 の ②連続始点指示 だけ
20. **残りを 1 つずつ** —— 変形（17）の ②包絡処理変形（外形線・線連結）と ④線記号変形、
    曲線（23）の ⑤手書線（と ⑥連続弧 の ①接する弧･線 指定）、
    帯の 電卓・範囲記憶・ｵﾌｾｯﾄ・HELP、ｵﾌﾟｼｮﾝ（29）の大きいところ
