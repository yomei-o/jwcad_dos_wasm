...
（一文字キーでも選べます）。**作図コマンドが 13 個動きます**——＋ ／ □ ○
「（」複線 線消 点 消去 線変更 文字 複写 移動。引いた図面は**保存**できて、
本物のＪＷ＿ＣＡＤ がそれを開いて同じ画面を描くところまで確かめてあります。
...
**いまできていること: 画面と、図面の読み書きと、作図コマンド 13 個と、読取。**
...
まだ無いのは残りの作図コマンド（変形・寸法・ハッチ・図形ほか 17 個）と、
...
13. **作図コマンドを 1 つずつ** —— いま 11 個。次は `③指定範囲` と、
    それを使う 複写・移動
