...
**いまできていること: 実行ファイルの構造の解明と、画面の下半分。**
ルート 618 関数とオーバーレイ 36 本（コードの 89.6%）を逆コンパイルし、
36 本それぞれが何担当かを割り出し、VGA のグラフィックコントローラと
線描画を C に移して絵が出るところまでです。
...
円弧はまだ多角形で代用しています（原典の `20a9:0e18` は 1,668 バイト・引数 10 個で、
度数で最大 5 分割して 8087 で計算するもの。未移植）。ただし**開始角・終了角・扁平率・
傾きはファイルから正しく読めていて**、扉の開きがちゃんと四分円になります。
...
2 行目の先頭 4 つが**線・円・点・文字列の個数**です。プログラム自身が出す
`線=%ld 円=%ld 点=%d 文字列=%d(%ld字)`（DGROUP 0x022e）と同じ順で、
これが鍵になりました。

| | 線 | 円 | 点 | 文字列 |
|---|---|---|---|---|
| `TEST7.JWC` | 4,083 | 5 | 106 | 18 |
| `SAMPLE2.JWC` | 1,084 | 98 | 46 | 10 |

個数が分かれば残りが読めます。**線は 22 バイト**（`float x0,y0,x1,y1` ＋ 属性 6）、
**円は 32 バイト**。
...
short ?;                 2   ほとんど 0
short start, start_frac; 4   開始角（度）と 1/10000 の端数
short end, end_frac;     4   終了角。start と同じなら全周
short tilt;              2   楕円の傾き（度）
byte  pen, type;         2   線の属性。線レコードの先頭 2 バイトと同じ
byte  rest[4];           4
```
...
個数がちょうど合わないと位置がずれるので、逆に個数が検算になります。

まだ分かっていないのは**データ本体のどこから幾何が始まるか**です。手前に
可変長のもの（バイト列からはレイヤ表に見えます）があって、サンプル 6 枚のうち
5 枚では 1,589、`SAMPLE2.JWC` だけ 1,621 でした。いまは「線も円も全部
もっともらしく読める最初の位置」を探しています。両方の個数を満たすという
条件はかなりきついので、手で数えた位置と一致します。
...
6. ~~画面の下半分~~ 済 — `src/vga.c`（グラフィックコントローラ）、`src/draw.c`（線）
7. 円・弧（`20a9:0e18`）と、文字・塗り
8. 図面ファイル `.JWC` の読み込み —— オーバーレイ 36。`SAMPLE*.JWC` が 13 枚ある
9. 入力 —— マウス（`INT 33h`）とキーボード、FEP
10. 起動から画面が出るまでを C で
11. 作図コマンドを 1 つずつ
12. ネイティブ（Win32 DIB）と WASM（`putImageData`）
