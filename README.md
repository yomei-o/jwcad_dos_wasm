...
（一文字キーでも選べます）。**作図コマンドが 21 個動きます**——＋ ／ □ ○
「（」複線 線消 点 消去 線変更 文字 複写 移動 線伸縮 線切断 コーナー連結
面取 ２線 中心線 分割 正多角形 測定。引いた図面は**保存**できて、本物のＪＷ＿ＣＡＤ がそれを
...
**いまできていること: 画面と、図面の読み書きと、作図コマンド 21 個と、読取と、日本語入力。**
...
まだ無いのは残りの作図コマンド（変形・寸法・ハッチ・図形ほか 9 個）と、
...
13. **作図コマンドを 1 つずつ** —— いま 21 個。残り 9 個
    （寸法・変形・ハッチ・曲線・円線接・図形・文編集・
    ｵﾌﾟｼｮﾝ・入出力）
