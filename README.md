...
（一文字キーでも選べます）。**作図コマンドが 11 個動きます**——＋ ／ □ ○
「（」複線 線消 点 消去 線変更 文字。引いた図面は**保存**できて、本物の
ＪＷ＿ＣＡＤ がそれを開いて同じ画面を描くところまで確かめてあります。
読み込みも描画も移植した C で、画面は当時と同じ 640×480 16 色
（VGA モード 12h）の 4 プレーンをそのまま展開しています。

**いまできていること: 画面と、図面の読み書きと、作図コマンド 11 個。**
...
まだ無いのは残りの作図コマンド（複写・移動・変形・寸法ほか）と、日本語入力です。
