...
同梱の図面をブラウザで開いて、拡大縮小と移動ができます。読み込みも描画も
移植した C で、画面は当時と同じ 640×480 16 色（VGA モード 12h）の
4 プレーンをそのまま展開しています。

**いまできていること: 実行ファイルの構造の解明と、図面を読んで描くところまで。**
ルート 618 関数とオーバーレイ 36 本（コードの 89.6%）を逆コンパイルし、
36 本それぞれが何担当かを割り出し、VGA のグラフィックコントローラ・線・円弧を
C に移し、`.JWC` の形を解いて同梱の図面 14 枚すべてを読めるようにしました。
まだ無いのは文字（発生器）と、作図コマンドと、マウスです。
...
10. **DOS/V エミュレータを正解にした画面比較**（下記）
11. 入力 —— マウス（`INT 33h`）とキーボード、FEP
12. 作図コマンドを 1 つずつ

## 検証の最終形 —— エミュレータと突き合わせる

移植が正しいかは、最終的には**本物と同じ絵が出るか**でしか決まりません。
そこで、DOS/V エミュレータの WASM 版で本物の `JW_CADV.EXE` を動かし、
同じ操作をしたときの画面をこの移植と突き合わせる、という形を目指しています。

そのための布石はもう打ってあります。

* **同じフォントを両方に持たせられる形にした。** 原典は
  `INT 15h AX=5000h` で DOS/V に字形を尋ねるので、エミュレータ側が
  `font/*.FNT` を同じ API で返せば、両者は同じ字形を描きます。
  FONTX2 を選んだのはこのためです
* **画面はプレーンのまま持っている。** `src/vga.c` は 640×480 の 4 プレーンを
  そのまま保持するので、エミュレータ側の VRAM とバイト単位で比べられます
* **比較の型はもうある。** `tools/check.sh` がネイティブと WASM を
  1 画素ずつ突き合わせています。相手がエミュレータに変わるだけです
