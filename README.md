...
（一文字キーでも選べます）。／ と □ と ○ なら作図領域を 2 回押すと線・矩形・
円が引けます。読み込みも描画も移植した C で、画面は当時と同じ 640×480 16 色
（VGA モード 12h）の 4 プレーンをそのまま展開しています。

**いまできていること: 画面と、図面と、作図の最初の 3 つ。**
...
|---|---|
| 図面なしの起動画面 | **0 画素差** |
| 図面 14 枚（640×480 まるごと） | 13 枚が **0 画素差**、TEST7 だけ 101（うち 95 は本物側のバグ） |
| メニュー 30 項目を押したあと | 丸数字 ①②③ 以外は一致（フォントに無い字） |
| ／ □ ○ で引いたあと | **0 画素差** |

まだ無いのは残りの作図コマンドと、読取（スナップ）と、日本語入力です。
...
## 本物と突き合わせる —— 14 枚のうち 13 枚が 1 画素も違いません
...
描いている画素は全部で 10 万を超えます。TEST7 に残る 101 画素は、**95 が
本物の側のバグ**です——線レコードは 22 バイト、データは `4046` から始まるので
`4046 + 22*2795 = 65536` ちょうど、そこでポインタが巻き戻って本物は 2795 番
から先の細かい影の線を描きません。残り 6 画素は縦の文字枠ひとつで、本物の
三角関数の 1e-5 ほどの誤差が出ています。どちらも [RESUME.md](RESUME.md) に
書いてあります。
