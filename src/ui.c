...
void jw_ui_default(JwUi *s)
{
    ...
    s->line_type = 1;
    s->dec[0] = s->dec[1] = 3;
    /* 測定の小数桁。**ここに置くのが要点**です——ページ側だけに
     * 3 を持たせていたので、同じ ui.c を使う検査は 0 桁で描き、
     * `6.41 ｍ` が `6 ｍ` になって 336 画素の差に見えていました。 */
    s->meas_dec = 3;
    /* **「前回と同じ」の欄**。本物は最初から数を持っています。
     * 同じ数が `jw_cmd_default`（src/cmd.c）にもあり、ページは
     * そちらを写しますが、`JwUi` を直に作る読み手はここを見ます。 */
    s->gap = 1000.0;                    /* 複線 の間隔 */
    s->gap_chamfer = 30.0;              /* 面取 の ③寸法 */
    s->gap_two[0] = s->gap_two[1] = 75.0;   /* ２線 の間隔 */
    s->divisions = 2;                   /* 分割 の [2] */
    s->ask_len = 1000.0;                /* 寸法・長さの欄 */
    s->ask_ang = 45.0;
    s->sine_cycle = 2000.0;             /* 曲線 ①ｻｲﾝ */
    s->sine_amp = 1000.0;
    s->sine_div = 100.0;
    s->ch_r = 1000.0;                   /* 曲線 ⑥連続弧 の ③半径 */
    s->spl_div = 5;                     /* 曲線 ③ｽﾌﾟﾗｲﾝ の区間 */
    s->tan_r = 1000.0;                  /* 円線接 ②接円半径 */
    s->tan_prev = 45.0;                 /* 円線接 ①接線 ④角度 */
    s->hen_dbl_gap = 100.0;             /* 変形 ③複線化 の間隔 */
    s->hen_dbl_cap = 1;                 /* 同 ④留線【有】 */
    s->hand_step = 4;                   /* 曲線 ⑤手書線 の作図ｽﾃｯﾌﾟ */
        ...
        for (i = 1; !s->top_item && !s->zukei && i <= s->stage; i++) {
            ...
            if (s->command == 23 && s->spl && i == s->stage
                ...
                if (i == 30) {
                    ...
                    sprintf(one, s->spl == 2 ? "\xcd\xde\xbc\xde\xaa\x8b\xc8\x90\xfc\x81\x9e     \x8en\x93_    \x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d" : "\xbd\xcc\xdf\xd7\xb2\xdd\x8b\xc8\x90\xfc\x81\x9e     \x8en\x93_    \x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d",
                            50 - s->spl_n);
                    jw_ui_text(v, 8, 1, 7, 0, one);
                    jw_ui_text(v, 73, 1, 7, 0, "[BS]\x91O\x8d\x80");
                } else if (i >= 31 && i <= 33) {
                    jw_ui_text(v, 1, 1, 7, 0, "[ESC]  ");
                    sprintf(one, s->spl == 2
                            ? (i == 31 ? "\xcd\xde\xbc\xde\xaa\x8b\xc8\x90\xfc\x81\x9f   \x92\xca\x89\xdf\x93_    \x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d" : i == 32 ? "\xcd\xde\xbc\xde\xaa\x8b\xc8\x90\xfc\x81\x9c \x92\xca\x89\xdf\x93_\x81" "E\x8fI\x93_\x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d" : "|\x87@\x93_\x8ew\x8e\xa6\x8fI\x97\xb9| \x81\x9c \x92\xca\x89\xdf\x93_\x81" "E\x8fI\x93_\x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d")
                            : (i == 31 ? "\xbd\xcc\xdf\xd7\xb2\xdd\x8b\xc8\x90\xfc\x81\x9f   \x92\xca\x89\xdf\x93_    \x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d" : i == 32 ? "\xbd\xcc\xdf\xd7\xb2\xdd\x8b\xc8\x90\xfc\x81\x9c \x92\xca\x89\xdf\x93_\x81" "E\x8fI\x93_\x8ew\x8e\xa6(L)free (R)Read  \x93_\x8e" "c\x90\x94 %2d"
                    ...
                    jw_ui_text(v, 1, 1, 7, 0, "[ESC]");
                    sprintf(one, s->spl == 2 ? "\xcd\xde\xbc\xde\xaa |\x87@\x8d\xec\x90}\x8aJ\x8en |\x87" "A\x8b\xe6\x8a\xd4\x95\xaa\x8a\x84\x90\x94(%d) |"
                            : "\xbd\xcc\xdf\xd7\xb2\xdd |\x87@\x8d\xec\x90}\x8aJ\x8en |\x87" "A\x8b\xe6\x8a\xd4\x95\xaa\x8a\x84\x90\x94(%d) |", s->spl_div);
                    jw_ui_text(v, 8, 1, 7, 0, one);
                } else {
                    int k, x;
            ...
            if (s->command == 26 && i == s->stage
                && (s->stage == 50 || s->stage == 57
                    || (s->stage >= 44 && s->stage <= 56)
                    || (s->stage >= 58 && s->stage <= 67))) {
                if (i == 50 || i == 57) {
                    jw_ui_text(v, 8, 1, 7, 0, i == 50 ? "|\x87@ \x82R\x93_ |\x87" "A \x82Q\x93_\x82\xc6\x82P\x90\xfc\xa5\x89~ |\x87" "B \x82P\x93_\x82\xc6\x82Q\x90\xfc\xa5\x89~ |\x87" "C \x82R\x90\xfc\xa5\x89~ |"
                                             : "\x90\xda\x91\xc8\x89~   |\x87@ \x82R\x93_ |\x87" "A \x95H\x8c`\x93\xe0\x90\xda |\x87" "B \x95\xbd\x8ds\x8el\x95\xd3\x8c`\x93\xe0\x90\xda |");
                    jw_ui_text(v, 73, 1, 7, 0, "[BS]\x91O\x8d\x80");
                } else {
                    /* 一つめを押したあとは桁 1 に [ESC]。行の頭の空白が
                     * 段ごとに 2 桁ずつ増えます（測定）。 */
                    if (s->tan_did || i == 45 || i == 46
                        || i == 48 || i == 49
                        || i == 52 || i == 53 || i == 55
                        || i == 56 || i == 59 || i == 60
                        || i == 62 || i == 63
                    ...
                    jw_ui_text(v, 8, 1, 7, 0,
                               i == 44 ? " \x82R\x93_\x82\xcc\x90\xda\x89~  \x91\xe6\x82P\x82\xcc\x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read " : i == 45 ? " \x82R\x93_\x82\xcc\x90\xda\x89~    \x91\xe6\x82Q\x82\xcc\x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read "
                               : i == 46 ? " \x82R\x93_\x82\xcc\x90\xda\x89~      \x91\xe6\x82R\x82\xcc\x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read " : i == 47 ? " \x82Q\x93_\x82\xc6\x82P\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~  \x91\xe6\x82P\x82\xcc\x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read "
                               : i == 48 ? " \x82Q\x93_\x82\xc6\x82P\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~    \x91\xe6\x82Q\x82\xcc\x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read " : i == 49 ? " \x82Q\x93_\x82\xc6\x82P\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~  \x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 51 ? " \x82P\x93_\x82\xc6\x82Q\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~  \x93_\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read " : i == 52 ? " \x82P\x93_\x82\xc6\x82Q\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~  \x91\xe6\x82P\x82\xcc\x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 53 ? " \x82P\x93_\x82\xc6\x82Q\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~    \x91\xe6\x82Q\x82\xcc\x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 " : i == 54 ? " \x82R\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~  \x91\xe6\x82P\x82\xcc\x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 55 ? " \x82R\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~    \x91\xe6\x82Q\x82\xcc\x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 " : i == 56 ? " \x82R\x90\xfc\xa5\x89~\x82\xcc\x90\xda\x89~      \x91\xe6\x82R\x82\xcc\x90\xfc\xa5\x89~\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 58 ? "\x82R\x93_\x91\xc8\x89~  \x8e\xb2\x82\xcc\x8en\x93_\x8ew\x8e\xa6 \x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read " : i == 59 ? "\x82R\x93_\x91\xc8\x89~    \x8e\xb2\x82\xcc\x8fI\x93_\x8ew\x8e\xa6 \x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read "
                               : i == 60 ? "\x82R\x93_\x91\xc8\x89~  \x93_ \x83}\x83" "E\x83X\x8ew\x8e\xa6 (L)free (R)Read "
                               : i == 61 ? " \x95H\x8c`\x93\xe0\x90\xda \x91\xc8\x89~  \x91\xe6\x82P\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 " : i == 62 ? " \x95H\x8c`\x93\xe0\x90\xda \x91\xc8\x89~    \x91\xe6\x82Q\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 63 ? " \x95H\x8c`\x93\xe0\x90\xda \x91\xc8\x89~      \x91\xe6\x82R\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 64 ? " \x95\xbd\x8ds\x8el\x95\xd3\x8c`\x93\xe0\x90\xda \x91\xc8\x89~  \x91\xe6\x82P\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 " : i == 65 ? " \x95\xbd\x8ds\x8el\x95\xd3\x8c`\x93\xe0\x90\xda \x91\xc8\x89~    \x91\xe6\x82Q\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 "
                               : i == 66 ? " \x95\xbd\x8ds\x8el\x95\xd3\x8c`\x93\xe0\x90\xda \x91\xc8\x89~      \x91\xe6\x82R\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 " : " \x95\xbd\x8ds\x8el\x95\xd3\x8c`\x93\xe0\x90\xda \x91\xc8\x89~        \x91\xe6\x82S\x82\xcc\x95\xd3\x82\xf0\x83}\x83" "E\x83X\x8ew\x8e\xa6 ");
                    jw_ui_text(v, 73, 1, 7, 0, "[BS]\x91O\x8d\x80");
                }
            }
            /* ②接円（半径と２条件）の道。 */
